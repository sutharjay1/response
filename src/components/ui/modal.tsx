...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-sidebar/40 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className,
    )}
    {...props}
    ref={ref}
  ...
  ),
  {
    variants: {
      side: {
        ...
        bottom:
          "inset-x-0 bottom-0 border-t lg:h-fit max-h-[90%] rounded-t-2xl data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
...
>(({ side = "bottom", className, children, ...props }, ref) => (
  <ModalPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(ModalVariants({ side }), className, "rounded-2xl")}
      {...props}
    >
      {children}
